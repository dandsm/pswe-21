...
CACHE_FILE = "vector_store_cache.pkl"  # archivo de caché en disco
...
def save_vector_store_to_cache(vector_store: List[Dict], cache_path: str) -> None:
    """Guarda el vector store (con embeddings) en disco."""
    with open(cache_path, "wb") as f:
        pickle.dump({"vector_store": vector_store}, f)
    print(f"Vector store guardado en caché: {cache_path}")


def load_vector_store_from_cache(cache_path: str) -> List[Dict]:
...
# ===== Bucle principal (CLI) =====
def main():
    # Si hay caché, lo usamos y nos ahorramos leer PDFs y calcular embeddings
    if os.path.exists(CACHE_FILE):
        print("Se encontró un caché de embeddings en disco.")
        use_cache = input(
            f"¿Desea usar el caché existente '{CACHE_FILE}'? (s/n): "
        ).strip().lower()
